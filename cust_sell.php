...
<script type='text/javascript'>
	function validate()
	{
		...
		if(form1.txtptype.value=="")
		{
			...
			{
				alert("Please Enter Only Characters in Property Type");
				form1.txtptype.focus();
				return false;
			}
		}
		...
		if(form1.txtcity.value=="")
		{
			...
			{
				alert("Please Enter Only Characters in Property City");
				form1.txtcity.focus();
				return false;
			}
		}
		...
		if(form1.txtprice.value=="")
		{
			...
			for(i=0;i<c.length;i++)
			{
				if(isNaN(c.charAt(i)))
				{
					alert('Dont Used Character or Special Symbols in Property Price');
					return false;
					break;
				}
			}
...
if(isset($_POST['btninsert']))
{
	...
	$query="insert into property_master values('$pid','$catid','$ptype','$add','$city','$desc','$price','$mypath','$area','$stype','0','$cid')";
	if(mysqli_query($con,$query))
	{
		
		echo "<script type='text/javascript'>";
		echo "alert('Property Uploaded');";
		echo "window.location.href='cust_sell_img.php?pid=$pid';";
		echo "</script>";
	}
}
...
if(isset($_POST['btnedit']))
{
	...
	if($_FILES['txtimg']['size']>0)
	{
		...
		$query="update property_master set category_id='$catid',property_type='$ptype',address='$add',city='$city',description='$desc',property_price='$price',property_img='$mypath',property_area='$area',type='$stype' where property_id='$pid'";
			...
			$query="update property_master set category_id='$catid',property_type='$ptype',address='$add',city='$city',description='$desc',property_price='$price',property_area='$area',type='$stype' where property_id='$pid'";
	}
	if(mysqli_query($con,$query))
	{
...
{
	$pid=$_REQUEST['did'];
	$query="update property_master set status='3' where property_id='$pid'";
	if(mysqli_query($con,$query))
	{
		
		echo "<script type='text/javascript'>";
		echo "alert('Property Sold');";
		echo "window.location.href='cust_sell.php';";
		echo "</script>";
	}
}
?>
	...
	<div class="row">
		...
		<form method="post" name="form1" class="contact" enctype="multipart/form-data">
							<div class="form-group has-feedback">
								<label for="email">Property Id</label>
			...
			<div class="form-group has-feedback">
				...
				<div class="form-group has-feedback">
					...
					<input type="file" name="txtimg" id="txtimg" placeholder="">
									...
									<?php
										$cid=$_SESSION['cid'];
										$qur1=mysqli_query($con,"select * from property_master where customer_id='$cid'");
										if(mysqli_num_rows($qur1)>0)
										{
											...
											while($q1=mysqli_fetch_array($qur1))
											{
												...
												echo "<td><a href='cust_sell.php?eid=$q1[0]'>EDIT</a></td>";
												echo "<td><a href='cust_sell.php?did=$q1[0]'>SOLD</a></td>";
												echo "<td><a href='cust_sell_img.php?pid=$q1[0]'>VIEW MORE IMAGES</a></td>";
